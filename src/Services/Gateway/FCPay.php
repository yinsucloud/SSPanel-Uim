...
class FCPay extends AbstractPayment
{
    ...
    {
        $this->fcpayId = $_ENV["fcpay_id"];
        $this->fcpayKey = $_ENV["fcpay_key"];
        $this->baseUrl = $gatewayBaseUrl;
    }

    private function getCallbackUrl()
    {
        return $_ENV['baseUrl'] . 'user/code';
    }

    private function getNotifyUrl()
    {
        return $_ENV['baseUrl'] . 'payment/notify';
    }
    ...
    public function purchase($request, $response, $args)
    {
        ...
        $pl = new Paylist();
        $pl->userid = $user->id;
        $pl->total = $price * $_ENV["fcpay_ex_rate"];
    ...
    public function getPurchaseHTML()
    {
        ...
        <form class="fcpay" name="fcpay" action="/user/code/fcpay" method="get">
            ...
            <p>当前入账汇率: <font size="4" color="#399AF2">' . $_ENV["fcpay_ex_rate"] . '</font>. （无论实际支付，入账金额 = 充值金额 X 入账汇率）</p>
            <br>
